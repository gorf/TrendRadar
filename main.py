...
def load_config():
    ...
    config = {
        ...
        # output 日期目录保留天数，避免仓库无限变大
        "OUTPUT_RETENTION_DAYS": config_data.get("output", {}).get(
            "retention_days", 7
        ),
    }
...
def cleanup_old_output_dirs(retention_days: Optional[int] = None) -> None:
    """
    清理 output 下过期的日期目录（形如 2026年08月15日）。
    只动日期文件夹，保留 .push_records 等非日期目录。
    """
    if retention_days is None:
        retention_days = CONFIG.get("OUTPUT_RETENTION_DAYS", 7)

    # 关闭清理：配置成 0 或负数就不删
    if retention_days is None or retention_days <= 0:
        print("output 保留天数未启用，跳过清理")
        return
    ...
    cutoff = get_beijing_time().date() - timedelta(days=retention_days)
    deleted = 0

    for item in output_root.iterdir():
        if not item.is_dir():
            continue
        # 只匹配「YYYY年MM月DD日」这种日期文件夹
        try:
            folder_date = datetime.strptime(item.name, "%Y年%m月%d日").date()
        except ValueError:
            continue
        ...
        if folder_date < cutoff:
            try:
                shutil.rmtree(item)
                deleted += 1
                print(f"已清理过期 output 目录: {item.name}")
            except Exception as e:
                print(f"清理 output 目录失败 {item.name}: {e}")

    if deleted:
        print(f"output 清理完成，共删除 {deleted} 个早于 {cutoff} 的日期目录")
    else:
        print(f"output 无需清理（保留最近 {retention_days} 天）")
