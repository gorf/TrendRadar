...
> 📖 **详细部署指南**：如果你需要详细的部署步骤说明，请查看 [DEPLOYMENT.md](DEPLOYMENT.md) 文档
...
5. **查看结果**:
   ...
   - `output` 默认只保留最近 **7** 天（见 `config/config.yaml` 的 `output.retention_days`），避免仓库越来越大导致 GitHub Actions 推送失败
   - 同时通过配置的机器人发送通知到你的群组
